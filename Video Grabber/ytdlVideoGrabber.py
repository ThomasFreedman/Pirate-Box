...
# Need to clean up by trapping exception but resume download process. Perhaps
...
def openSQLiteDB(columns, dbFile):
    ...
    # Initialize DOWNLOAD_IP with IP address used on last run
        ...
        # Add new column "vsize" to database if it doesn't exist
        try:
            conn.execute("ALTER TABLE IPFS_HASH_INDEX ADD COLUMN 'vsize' TEXT;")
        except Exception:
            pass            # Ignore errors. Will remove this code in time
    return conn
...
def publishDB(file):
    ...
    if len(newDBhash) == 46:
        return newDBhash        # Can't do this until a ipNs name  is assigned
        lst = []
...
#
# The grupe quota limits are not used in this Pirate Box version of the
# grabber. Instead only the json metadata is preserved and the download
# is deleted after a hash confirms it was added to IPFS.
def processVideo(file):
    ...
    except Exception as e:                # Log errors that may have occurred
...
def ytdlProcess(config, conn):
    ...
    with youtube_dl.YoutubeDL(dlOpts) as ydl:
        for grupe in grupeList:
            ...
            # youtube-dl uses its' history for this purpose also
